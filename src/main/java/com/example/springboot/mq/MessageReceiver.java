...
public class MessageReceiver implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        if (queueConfigurations.isEmpty()) {
            ...
            for (QueueConfiguration queueConfig : queueConfigurations) {
                logQueueDetails(queueConfig);
                setupQueueListenerAndProcessor(queueConfig);
            }
        }
    }

    private void logQueueDetails(QueueConfiguration queueConfig) {
    ...
    private void setupQueueListenerAndProcessor(QueueConfiguration queueConfig) {
        try {
            // Create a JMS Queue object using the queue name
            Queue queue = new MQQueue(queueConfig.getQueueName());

            // Configure the properties for the connection factory
            setConnectionFactoryProperties(queueConfig);

            // Create a JmsListenerEndpoint for the queue
            SimpleJmsListenerEndpoint endpoint = new SimpleJmsListenerEndpoint();
            endpoint.setId(new SimpleJmsListenerEndpointId(queueConfig.getQueueName()));
            endpoint.setDestination(queue);

            // Set the message listener for processing the received messages
            endpoint.setMessageListener(message -> processReceivedMessage(queueConfig, message));
    ...
    private void setConnectionFactoryProperties(QueueConfiguration queueConfig) throws JMSException {
        // Set the connection factory properties based on the queue configuration
    ...
    private void processReceivedMessage(QueueConfiguration queueConfig, String message) {
        // Handle the received message asynchronously
        System.out.println("Received message from " + queueConfig.getQueueName() + ": " + message);
        // Additional processing logic for the received message
    }
